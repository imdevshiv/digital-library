...
const {
  ...
  getDownloadURL,
  deleteObject,
} = require("firebase/storage"); // getting required services from firestore
...
app.get("/login", async (req, res) => {
  ...
  if (req.session.isAuth === true) {
    const foundUser = await User.findOne({ _id: req.session.user_ID }).select(
      "-password"
    );
...
app.post(
  ...
  async (req, res) => {
    if (req.session.isAuth === true && req.session.userType === "Teacher") {
      ...
      const foundUser = await User.findOne({ _id: req.session.user_ID }).select(
        "-password"
      ); // gives all things without password
...
app.get("/view/docs/sem/:semNum", async (req, res) => {
  ...
  if (req.session.isAuth === true) {
    const finduser = await User.findOne({ _id: req.session.user_ID }).select(
      "-password"
    );
    const allRelatedSemesterDocs = await Document.find({ relatedSem: semNum }); // all docs for corresponding semester
    const selfUploadedDocs = await allRelatedSemesterDocs.filter(
      (individualDoc) => {
        return individualDoc.userId.toString() === finduser._id.toString();
      }
    );
    //other teacher docs
    const otherTeacherDocs = await allRelatedSemesterDocs.filter(
      (individualDoc) => {
        return individualDoc.userId.toString() != finduser._id.toString();
      }
    );
    res.render("alldocsviewpage", {
      semNum,
      user: finduser,
      documents: allRelatedSemesterDocs,
      selfUploadedDocs,
      otherTeacherDocs,
    });
  } else {
...
app.post("/delete/doc/sem/:semNum/:docId", async (req, res) => {
  ...
  const documentsFolderRef = ref(
    documentsRef,
    `semester${semNum}/${docGivenName}`
  );

  if (
    findThatDoc.docType === "image/png" ||
    findThatDoc.docType === "image/jpeg"
  ) {
    // delete image
    deleteObject(imagesFolderRef)
      .then(async () => {
    ...
    //delete document
    deleteObject(documentsFolderRef)
      .then(async () => {
        // File deleted successfully -- then delete in mongoDB
        await Document.findByIdAndDelete(docId);
        res.redirect(`/view/docs/sem/${semNum}`);
...
cron.schedule('*/15 * * * *', async () => { 
    try {
        const response = await axios.get('https://digital-library-0ktu.onrender.com/');
        console.log('Endpoint hit successfully:', response.status);
    } catch (error) {
        console.error('Error hitting the endpoint:', error.message);
    }
});

console.log("15-minute cron job set up to keep the app active.");
